...
namespace TPS {
    public class PlayerController : MonoBehaviour
    {
        ...
        {
            //Wallrun
            if (x > 0.2f && isWallRight) StartWallrun();
            if (x < -0.2f && isWallLeft) StartWallrun();
        }
        ...
        public void OnJump(CallbackContext context)
        {
            ...
            if (context.performed) startJump = true;
        ...
        private void MyInput()
        {
            ...
            //Double Jumping
            if (startJump && !grounded && doubleJumpsLeft >= 1)
            {
                Jump();
                doubleJumpsLeft--;
            }
            ...
            //Climbing
            if (Physics.Raycast(transform.position, transform.forward, 1, whatIsLadder) && y > .9f)
                Climb();
        ...
        private void StartCrouch()
        {
            ...
            if (rb.velocity.magnitude > 0.5f)
            {
                if (grounded)
                {
                    rb.AddForce(transform.transform.forward * slideForce);
                }
            }
        }
        ...
        private void Jump()
        {
            ...
            if (isWallRunning)
            {
                ...
                //normal jump
                if (isWallLeft && x < -0.2f || isWallRight && x > 0.2f)
                {
                    rb.AddForce(Vector2.up * jumpForce * 1.5f);
                    rb.AddForce(normalVector * jumpForce * 0.5f);
                }
                ...
                //sidwards wallhop
                if (isWallRight || isWallLeft && x < -0.2f || x > 0.2f) rb.AddForce(-transform.up * jumpForce * 1f);
                if (isWallRight && x < -0.2f) rb.AddForce(-transform.right * jumpForce * 3.2f);
                if (isWallLeft && x > 0.2f) rb.AddForce(transform.right * jumpForce * 3.2f);
        ...
        private void Dash()
        {
            ...
            dashStartVector = transform.forward;
        ...
        private void SonicSpeed()
        {
            ...
            if (timePassedSonic >= timeBetweenNextSonicBoost)
            {
                ...
                {
                    maxSpeed += 5;
                    rb.AddForce(transform.forward * Time.deltaTime * sonicSpeedForce);
                }
                timePassedSonic = 0;
            }
        }
        private void SideFlash(bool isRight)
        {
            RaycastHit hit;
            //Flash Right
            if (Physics.Raycast(transform.position, transform.right, out hit, flashRange) && isRight)
            {
                transform.position = hit.point;
            }
            else if (!Physics.Raycast(transform.position, transform.right, out hit, flashRange) && isRight)
        ...
        private void StartRocketBoost()
        {
            ...
            */
            ...
            rb.AddForce(transform.forward * rocketForce * Time.deltaTime * 1f);
        ...
        private void StopRocketBoost()
        {
            ...
            {
                rb.AddForce(transform.forward * rocketForce * -.2f);
                rb.AddForce(Vector3.up * rocketForce * -.4f);
            }
            else
            {
                rb.AddForce(transform.forward * rocketForce * -.2f * rocketTimer);
        ...
        private void Climb()
        {
            ...
            if (rb.velocity.magnitude < maxClimbSpeed)
                rb.AddForce(transform.up * climbForce * Time.deltaTime);
        ...
        private void Look()
        {
            ...
            desiredX = rot.y + lookX * sensitivity * Time.fixedDeltaTime * sensMultiplier;

            //Rotate, and also make sure we dont over- or under-rotate.
            xRotation -= lookY * sensitivity * Time.fixedDeltaTime * sensMultiplier;
            xRotation = Mathf.Clamp(xRotation, -90f, 90f);

            //Perform the rotations
            //transform.localRotation = Quaternion.Euler(xRotation, desiredX, wallRunCameraTilt);
            transform.localRotation = Quaternion.Euler(0f, desiredX, wallRunCameraTilt);
            transform.transform.localRotation = Quaternion.Euler(0, desiredX, 0);
        ...
        private void CounterMovement(float x, float y, Vector2 mag)
        {
            ...
            {
                rb.AddForce(moveSpeed * transform.transform.right * Time.deltaTime * -mag.x * counterMovement);
            }
            if (Math.Abs(mag.y) > threshold && Math.Abs(y) < 0.05f || (mag.y < -threshold && y > 0) || (mag.y > threshold && y < 0))
            {
                rb.AddForce(moveSpeed * transform.transform.forward * Time.deltaTime * -mag.y * counterMovement);
            }
        ...
        {
            float lookAngle = transform.transform.eulerAngles.y;
